...
router = APIRouter(prefix="/projects/{project_id}/plugins", tags=["Project Plugins"])


@router.get("", response_model=PaginatedResponse[ProjectPluginResponse])
async def list_plugins(
    project_id: uuid.UUID,
    db: DBSession,
    page: int = Query(default=1, ge=1),
    size: int = Query(default=50, ge=1, le=100),
    tech_stack_id: uuid.UUID = Query(...),
):
    return await project_plugin_controller.list_plugins(project_id=project_id, page=page, size=size, db=db, tech_stack_id=tech_stack_id)


@router.get("/{plugin_id}", response_model=ProjectPluginResponse)
...
@router.post("", response_model=ProjectPluginResponse, status_code=status.HTTP_201_CREATED)
...
@router.patch("/{plugin_id}", response_model=ProjectPluginResponse)
